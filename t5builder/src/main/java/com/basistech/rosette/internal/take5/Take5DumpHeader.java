...
public final class Take5DumpHeader {
    ...
    private void dump() {
        ...
        dumpDictionary(t5Buffer, new PrintWriter(new OutputStreamWriter(System.out, Charsets.UTF_8), true));
    }

    // checkstyle hates long functions.
    //CHECKSTYLE:OFF
    static void dumpDictionary(ByteBuffer buffer, PrintWriter pw) {
        ...
        pw.format("value_data: %d\n", ints.get(Take5Format.HDR_VALUE_DATA));
        ...
        final int engine = ints.get(Take5Format.HDR_FSA_ENGINE);
        ...
        if (maxVersion >= Take5Format.VERSION_5_4) {
            ...
            for (int x = 0; x < epCount; x++) {
                dumpOneEntrypoint(buffer, x, maxVersion, pw);
            }
        }
    }

    private static int getNetworkInt(ByteBuffer buffer, int intOffset) {
    ...
    private static void dumpOneEntrypoint(ByteBuffer buffer, int epx, int maxVersion, PrintWriter pw) {
        IntBuffer ints = buffer.asIntBuffer();
        ...
        pw.format("  state_state: %d\n", buffer.getInt(epBase + (Take5Format.EPT_STATE_START * 4)));
