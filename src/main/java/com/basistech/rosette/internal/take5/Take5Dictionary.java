...
public class Take5Dictionary {
    ...
    public Take5Match matchExact(String data) throws Take5Exception {
        ...
        int c = take5Search(data.toCharArray(), 0, data.length(), match, null);
        if (c == 0) {
            return null;
        }
    ...
    public Take5Match matchExact(char[] data, int offset, int length) throws Take5Exception {
        Take5Match match = new Take5Match();
        int c = take5Search(data, 0, length, match, null);
        if (c == 0) {
            return null;
        }
    ...
    /**
     * Searches for a match in the dictionary of exactly the string provided. If such a match is found, 
     * the return value is true and the result object is filled in. Otherwise, the return value is false
     * and the result object contents are undefined.
     * The behavior of this function when a non-zero skipBits array is specified is undefined.
     * 
     * @param data buffer containing the key.
    ...
    public boolean matchExact(char[] data, int offset, int length, Take5Match result) throws Take5Exception {
        result.reset();
        int c = take5Search(data, 0, length, result, null);
        if (c == 0) {
            return false;
        }
    ...
    /**
     * Searches for the longest string matching the given key, specified by a buffer, offset, and length.
     * The Take5Match object associated with that
     * string is returned, otherwise null.
     * @param data buffer containing the key.
     * @param offset offset of the key in the buffer.
     * @param length length of the key.
    ...
    public Take5Match matchLongest(char[] data, int offset, int length) throws Take5Exception {
        Take5Match match = new Take5Match();
        int c = take5Search(data, offset, length, match, null);
        if (c == 0) {
            return null;
        }
        return match;
    ...
    public int matchMultiple(String data, Take5Match[] matches) throws Take5Exception {
        int c = take5Search(data.toCharArray(), 0, data.length(), null, matches);
        return c > matches.length ? matches.length : c;
    }
    
    /**
     * Finds all possible matches for prefixes of the given key,
     * specified by a buffer, offset, and length.
     * The caller controls the number of matches returned via the size of the 'matches' array.
     * 
     * @return the number of valid results.
     */
    public int matchMultiple(char[] data, int offset, int length, Take5Match[] matches) throws Take5Exception {
        int c = take5Search(data, offset, length, null, matches);
        return c > matches.length ? matches.length : c;
    }
     ...
     * @throws Take5Exception
     */
    int take5Search(char[] input, int offset, int length, Take5Match match, Take5Match[] matches) throws Take5Exception {
        return take5Search(input, offset, length, match, matches, stateStart, indexStart);
    }
    ...
    public Take5Match advanceByChar(Take5Match m, char c) throws Take5Exception {
        ...
        take5Search(cA, 0, 1, ret, null, m.state, m.index);
        if (ret.length == 0) {
            return null;
        }
        return ret;
     ...
     * If match is not null then a search for
     * the longest match is performed, the length of that match is returned. If matches is not null then a
     * search for multiple matches, up to the length of matches, is performed with the number of successful
     * matches returned. If none or both of match and matches are null then an IllegalArgumentException is
     * thrown.
     * <br/>
     * Only one of 'match' and 'matches' may be non-null.
     ...
     * @param startState a start state (see {@link #advanceByChar(Take5Match, char)}.
     * @param startIndex a start index (see {@link #advanceByChar(Take5Match, char)}.
     * @return the number of matches.
     * @throws Take5Exception
     */
    public int take5Search(char[] input, int offset, int length, Take5Match match, Take5Match[] matches, int startState, int startIndex)
        throws Take5Exception {
        ...
        loop: for (cnt = offset; cnt < limit; cnt++) {
            ...
            if ((state & 1) != 0) {
                ...
                if (match != null) {
                    ...
                    matches[out_cnt].length = cnt - offset;
